...
/**
 * Modern OD Application form with premium aesthetic, undecorated rounded window.
 */
public class ODForm extends JDialog {
    private final Integer id;
    private final Runnable callback;
    private final DatabaseManager db = DatabaseManager.INSTANCE;
    private JTextField empIdField, fromField, toField, daysField, locationField, purposeField;
    private JComboBox<String> typeCombo, statusCombo;
    private Point initialClick;

    public ODForm(JFrame parent, Integer id, Runnable callback) {
        super(parent, id == null ? "New OD Application" : "Edit OD Request #" + id, true);
        this.id = id;
        ...
        setSize(480, 600);
        UIHelper.centerWindow(this, 480, 600);
        buildUI();
        if (id != null) loadData();
        setVisible(true);
    }
    ...
    private void buildUI() {
        ...
        JLabel iconLbl = new JLabel(new com.formdev.flatlaf.extras.FlatSVGIcon("icons/od_request.svg", 32, 32));
        hdr.add(iconLbl);
        
        JLabel title = new JLabel(id == null ? "On Duty Application" : "Update OD Request");
        ...
        addField(form, "OD Category", typeCombo = combo("Official Trip", "Client Visit", "Field Work", "Training", "Emergency"));
        ...
        addField(form, "Location", locationField = tf("Client site or city"));
        addField(form, "Purpose / Reason", purposeField = tf("Reason for travel"));
        ...
        JButton saveBtn = UIHelper.makeButton(id == null ? "Submit Request" : "Save Changes", UIHelper.SUCCESS, "check.svg");
    ...
    private void loadData() {
        try {
            ...
            if (r != null) {
                ...
                locationField.setText(DatabaseManager.str(r, "location"));
                purposeField.setText(DatabaseManager.str(r, "purpose") != null ? DatabaseManager.str(r, "purpose") : DatabaseManager.str(r, "reason"));
    ...
    private void save() {
        ...
        try {
            ...
            if (id == null) {
                db.execute("INSERT INTO od_requests (emp_id, od_type, od_from, od_to, od_days, location, purpose, reason, status, applied_on) VALUES (?,?,?,?,?,?,?,?,?,NOW())", 
                    eid, typeCombo.getSelectedItem(), isoFrom, isoTo, days, loc, purp, purp, status);
            } else {
                db.execute("UPDATE od_requests SET emp_id=?, od_type=?, od_from=?, od_to=?, od_days=?, location=?, purpose=?, reason=?, status=? WHERE id=?", 
                    eid, typeCombo.getSelectedItem(), isoFrom, isoTo, days, loc, purp, purp, status, id);
            }
            ...
            UIHelper.showSuccess(this, "OD Request saved successfully!");
            callback.run(); 
            dispose();
        } catch (Exception e) {
